...
export const ToolbarDisabledContext = React.createContext(false);

export const useToolbarDisabled = (): boolean => {
	return React.useContext(ToolbarDisabledContext);
};
...
export const Toolbar = React.forwardRef<HTMLDivElement, ToolbarProps>(
	(
		{ bIsEnabled = true, children, className, variant = "fixed", ...props },
		ref,
	) => {
		...
		// When disabled (e.g. thread panel open), keep toolbar visible but non-interactive; only hide when visibility says so and we're enabled
		if (!isVisible && bIsEnabled) return null;

		return (
			<ToolbarDisabledContext.Provider value={!bIsEnabled}>
				<div
					aria-label="toolbar"
					className={cn(`tiptap-toolbar ${className || ""}`, {
						"cursor-not-allowed [&_*]:cursor-not-allowed [&_*]:pointer-events-none":
							!bIsEnabled,
					})}
...
export const ToolbarGroup = React.forwardRef<HTMLDivElement, BaseProps>(
	({ children, className, ...props }, ref) => {
		...
		const toolbarDisabled = React.useContext(ToolbarDisabledContext);

		// When toolbar is disabled (e.g. thread panel open), keep group visible so toolbar content stays on screen
		if (!isVisible && !toolbarDisabled) return null;
