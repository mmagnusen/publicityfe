...
function ListPagination({
	...
	const totalPages = Math.max(
		1,
		Math.ceil(totalCount / MEDIA_OUTLETS_PER_PAGE),
	);
	...
	const prevHref =
		currentPage > 1
			? `/admin/media-outlets?page=${currentPage - 1}`
			: undefined;
	const nextHref =
...
export function MediaOutletsList() {
	...
	const { data, error, isLoading } = useMediaOutlets(currentPage);
	const list = useMemo(() => normalizeMediaOutletListResponse(data), [data]);
				...
				{!authenticationChecked || (isLoading && !data) ? (
					...
					<Text variant="error">
						{accessDenied
							...
							: "Could not load media outlets. Check the API is available and try again."}
					</Text>
				) : (
					<>
						<Text variant="stat-label">
							{list.count} media outlet{list.count === 1 ? "" : "s"}
						</Text>

						{list.results.length === 0 ? (
							<Text variant="center-sm">No media outlets yet.</Text>
						) : (
							<ul className="mt-4 divide-y divide-gray-200">
								{list.results.map((outlet) => {
									const createdLabel = formatDate(outlet.created_at);
										...
										>
											<div className="min-w-0">
												...
												<a
													...
													className="mt-1 block truncate text-sm text-gray-600 underline-offset-2 hover:text-black hover:underline"
												>
													{outlet.website_url}
												</a>
												{createdLabel ? (
											...
											<Button
												href={`/admin/media-outlets/${outlet.pk}`}
												size="small"
